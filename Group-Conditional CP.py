...
class GroupConditionalConformalPredictor:
    ...
    def calibrate(self, probs, true_labels, group_labels):
        ...
        for g in np.unique(group_labels):
            ...
            qhat_g = np.quantile(group_scores, q_level, interpolation='higher')
            self.group_qhats[g] = qhat_g
        return self.group_qhats

    def predict(self, probs, group_labels):

        prediction_sets = []
        for i, prob in enumerate(probs):
            g = group_labels[i]
            qhat = self.group_qhats.get(g, None)
            if qhat is None:
                raise ValueError(f"Group {g} not seen in calibration!")
            threshold = 1 - qhat
            pred_set = [j for j, p in enumerate(prob) if p >= threshold]
            if not pred_set:
                pred_set = [0, 1]
            prediction_sets.append(pred_set)
        return prediction_sets

    def evaluate_coverage(self, prediction_sets, true_labels, group_labels):
        group_labels = np.array(group_labels)
        coverage = {}
        for g in np.unique(group_labels):
            idx = (group_labels == g)
...
#### for race
    cp = GroupConditionalConformalPredictor(alpha=0.1)
    qhat = cp.calibrate(calib_probs, y_calib,race_calib) #q_hat
    prediction_sets = cp.predict(test_probs,group_labels=race_test) ##cp interval for test set
    print("\nGroup-specific qhat values:")
    for group_idx, qhat in qhat.items():
        # group_name = "Female" if group_idx == 0 else "Male" 
        print(f"  {group_idx} (group {group_idx}): qhat = {qhat:.4f}")
...
### if would like to see performance of gender use this version 

    # cp = GroupConditionalConformalPredictor(alpha=0.1)
    # qhat = cp.calibrate(calib_probs, y_calib,gender_calib) #q_hat
    # prediction_sets = cp.predict(test_probs,group_labels=gender_test) ##cp interval for test set
    # print("\nGroup-specific qhat values:")
    # for group_idx, qhat in qhat.items():
    #     # group_name = "Female" if group_idx == 0 else "Male"  
    #     print(f"  {group_idx} (group {group_idx}): qhat = {qhat:.4f}")
